...
**목적**: DB 접속 방식 변경 이후 챗봇 측에서 발생하던 런타임 오류를 해결하기 위해 필수 라이브러리를 보강함.

#### 주요 변경 사항

1. **`greenlet` 라이브러리 추가 (`requirements.txt`)**
   - 비동기 SQLAlchemy 환경에서 동작 시 필요한 `greenlet==3.1.1`이 누락되어 챗봇 서비스 쪽에 오류가 발생하던 문제를 해결함.
   - `requirements.txt` 파일에 해당 라이브러리를 명시적으로 추가하여 정상 작동을 확인함.
