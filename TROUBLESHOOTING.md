...
### 1. RunPod Serverless Endpoint 삭제 불가
- **현상**: `runpod` 파이썬 라이브러리(v1.8.1)에 `delete_endpoint` 함수가 탑 레벨에 노출되어 있지 않아, 스크립트에서 생성한 엔드포인트를 코드로 삭제할 수 없음.
- **원인**: SDK의 기능 누락 또는 문서 불일치.
- **해결**: **GraphQL Mutation 직접 호출**.
  - `runpod.api.ctl_commands.run_graphql_query`를 임포트하여 `deleteEndpoint(id: "...")` 쿼리를 직접 전송하는 `terminate_endpoint` 함수 구현.

### 2. 워커 배정 실패 (Worker Allocation Stuck)
- **현상**: 엔드포인트를 생성했지만 "Workers: 0" 상태에서 변하지 않고 로그도 남지 않음.
- **원인**: 
  1. `gpu_ids` 파라미터에 "NVIDIA GeForce RTX 4090"이라는 Full Name을 입력하여 인식 실패.
  2. `locations="EU"` 등으로 지역을 강제하여, 해당 지역에 GPU 재고가 없을 경우 무한 대기.
- **해결**: **Correct GPU ID & Global Allocation**.
  - GPU ID를 RunPod 내부 식별자인 **`ADA_24`** (RTX 4090)로 수정.
  - `locations` 파라미터를 제거하여 전 세계 리전 중 가용한 곳에 자동 배정되도록 변경.

### 3. Idle Timeout 설정 오류 (500 Error)
- **현상**: Endpoint 생성 시 `500 Internal Server Error` 발생.
- **원인**: `idle_timeout`을 12시간(`43200`초)으로 설정했으나, Serverless의 최대 허용치는 **1시간(`3600`초)**임.
- **해결**: `idle_timeout=3600`으로 하향 조정.

### 4. 템플릿 환경변수와 스크립트 중복
- **현상**: `HF_TOKEN` 등 환경변수를 스크립트에서 주입(`env={...}`)하려고 시도했으나 복잡도 증가.
