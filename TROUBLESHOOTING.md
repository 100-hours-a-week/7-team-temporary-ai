...
### 2. 챗봇 모델 Fallback 테스트 실패 (AssertionError: gemini-3-flash-preview)
- **현상**: 전체 테스트 실행 시 `tests/test_chat_service.py`의 `test_generate_task_fallback_logic`이 `AssertionError: assert 'gemini-3-flash-preview' == 'gemini-2.5-flash'` 에러와 함께 실패함.
- **원인**: `chat_service.py`의 구현 상 3회 재시도 실패 후 Fallback 되는 모델이 `gemini-3-flash-preview`로 설정되어 있으나, 테스트 코드에서는 `gemini-2.5-flash`를 기대값으로 하드코딩하고 있었음. (최근 모델 정책 변경 시 테스트 코드가 미처 업데이트되지 않음)
- **해결**: **Test Assertion Synchronization**.
  - 테스트 코드의 기대값을 실제 서비스 로직과 동일하게 `gemini-3-flash-preview`로 수정하여 검증 정합성 확보.

### 3. 챗봇 스트리밍이 뭉떵이로 나오거나 끊기는 현상
- **현상**: 네트워크 상황이 좋을 때는 텍스트가 한꺼번에 우르르 나오고, 안 좋을 때는 한참 멈췄다가 몰아서 나오는 등 "타이핑" 느낌의 사용자 경험(UX)이 저해됨.
- **원인**: Gemini API는 내부 토큰 생성 속도에 맞춰 청크를 보내주지만, 우리 서버는 그 청크를 받은 즉시(뭉치 모양 그대로) 클라이언트에 전달하고 있었음.
- **해결**: **Server-Side Word Splitting & Throttling**.
  - 받은 청크를 즉시 보내지 않고, 어절 단위로 다시 쪼갠 뒤 각 어절마다 `0.05초`씩 쉬어가며 큐에 적재함.
  - 이를 통해 네트워크 지연과 무관하게 사용자에게는 일정한 속도로 챗봇 응답이 생성되는 것처럼 보이는 개선 효과를 얻음.
